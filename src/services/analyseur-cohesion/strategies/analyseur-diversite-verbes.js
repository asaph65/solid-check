...
class AnalyseurDiversiteVerbes {
    ...
    executer(contenu) {
        ...
        while ((correspondance = regex.exec(contenu)) !== null) {
            ...
            if (nomFonction && !MOTS_CLES_IGNORES.has(nomFonction)) {
                this._classerFonction(nomFonction, categoriesTrouvees, verbesParCategorie);
            }
        }

        if (categoriesTrouvees.size > 3) {
            this._ajouterViolation(violations, categoriesTrouvees, verbesParCategorie);
        }

        return violations;
    }

    _classerFonction(nom, categoriesTrouvees, verbesParCategorie) {
        for (const [categorie, verbes] of Object.entries(AnalyseurDiversiteVerbes.CATEGORIES_VERBES)) {
    ...
    _ajouterViolation(violations, categories, verbesMap) {
